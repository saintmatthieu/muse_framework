...
TEST_F(AudioPlugins_RegisterAudioPluginsScenarioTest, ValidatePluginAsync_SecondPluginIsNotQueuedBehindAHungOne)
{
    ...
    EXPECT_CALL(*m_process, execute(m_appPath, expectedValidationArgs(hungPath), _, _))
    ...
    EXPECT_CALL(*m_process, execute(m_appPath, expectedValidationArgs(otherPath), _, _))
    .WillOnce(Return(0));
