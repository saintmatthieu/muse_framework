...
struct RegisterAudioPluginsScenario::AsyncScan {
    ...
    std::string appPath;
    std::string crashDumpsDir; // for --crash-dumps-dir, computed on the main thread
...
void RegisterAudioPluginsScenario::ensureAsyncScan()
{
    ...
    m_asyncScan->crashDumpsDir = (globalConfiguration()->userAppDataPath() + "/logs/dumps-plugin-validation").toStdString();
}
    ...
    for (int64_t i = 0; i < count; ++i) {
        ...
        scan->workers.emplace_back([this, scan]() {
            ...
            while (!m_shuttingDown.load()) {
                ...
                // same crash reporting as the synchronous path (processPluginsRegistration)
                const std::string serverUrl = MUSE_MODULE_AUDIOPLUGINS_CRASHREPORT_URL;
                if (!serverUrl.empty()) {
                    args.insert(args.end(), { "--crash-dumps-dir", scan->crashDumpsDir });
                    args.insert(args.end(), { "--crash-server-url", serverUrl });
                }

                const int code = process()->execute(scan->appPath, args,
